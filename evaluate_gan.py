...
# --- TARGET METRICS ---
# These are general targets for symbolic music features
TARGET_STATS = {
...
def analyze_generated_midi(filepath):
    """Extracts Pitch, Velocity, and Note Density from a MIDI file."""
    try:
...
def evaluate_gan(device):
    ...
    # Ensure temp dir exists and clean it up if you want fresh results
    ...
    # Initialize FeatureEncoder with correct hidden dims from config
    ...
    # Initialize Generator with correct dimensions
    G = Generator(
        noise_dim=cfg['NOISE_DIM'], 
        latent_dim=cfg['LATENT_DIM'], 
        mode=cfg.get('INTEGRATION_MODE', 'warm_start'), 
        max_notes=cfg['MAX_NOTES'], 
        note_dim=cfg['NOTE_DIM'], # This should be 5
    ...
    print(f"[INFO] Loading weights from {GAN_CHECKPOINT}")
    ckpt = torch.load(GAN_CHECKPOINT, map_location=device)
    
    # Handle loading logic
    if 'G' in ckpt: G.load_state_dict(ckpt['G'])
    else: G.load_state_dict(ckpt)
    
    if 'E_num' in ckpt: E_num.load_state_dict(ckpt['E_num'])
    ...
    # Target Vectors (Valence/Arousal/etc)
    emotion_vectors = {
    ...
    print(f"[INFO] Analyzing {num_samples} samples per emotion...")
    ...
    for emo_name in emotions:
        ...
        with torch.no_grad():
            ...
            # --- CRITICAL: APPLY TIMING CORRECTION ---
            # Apply the same fix used in test_gan.py to get fair metrics
            if emo_name.lower() in ["sad", "calm"]:
                gen_notes[:, :, 3] += 0.4
                gen_notes = torch.clamp(gen_notes, -1.0, 1.0)
            ...
            for i in range(num_samples):
                ...
                # Handle 5-Channel Output (Slice to 4)
                sample_data = gen_notes_cpu[i]
                ...
                # We overwrite files to ensure we are analyzing the current model state
                save_piano_roll_to_midi(
                    sample_data, fpath, 
                    instrument_name=instrument, bpm=bpm, scale=scale, root_key=root
                )
